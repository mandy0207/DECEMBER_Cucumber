...
public class AddBookStepDefinition {
	...
	public void user_sends_post_request_to_add_book_with_unique_creds() {
		...
		addBookresponse=given().spec(CreateSpec.makeRequestSpec(ApiResources.baseUrl.getResource(), ContentType.JSON)).body(book).
				when().post(ApiResources.postBook.getResource()).then().extract().response();
	}
